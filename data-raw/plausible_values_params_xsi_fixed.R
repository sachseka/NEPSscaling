...
items <- c("maa2q071_sc5s1_c", "mas1r092_c", "mas1v093_c",
...
x <- c(-1.206, 1.748, -0.822, 0.450, -2.203, -0.646, 0.461, 0.344, -0.863,
         -0.472, -0.768, 0.710, -2.458, -1.279, 0.147, -1.212, -1.754, -0.522,
         -0.147, 1.171)
x51 <- data.frame(item=1:length(x), x)
rownames(x51) <- items
x52 <- xsi
...
x <- c(-0.527, -1.803, 1.550, -2.248, -0.813, 0.702, -0.229, -2.019, -0.215,
         0.691, -0.349, 1.481, -0.086, -1.195, 1.579, -0.124, -0.075, -1.120,
         -0.824, -3.183, 1.007)
x61 <- data.frame(item=1:length(x), x)
rownames(x61) <- items
x62 <- xsi
...
# SC5
items <- c("res10110_c", "res1012s_c", "res10130_c", "res10140_c",
...
x <- c(-1.154, -4.048, -3.441, -0.793, 0.124, -1.172, -2.441, -0.162, -2.538,
...
xsi <- readODS::read_ods(path = "../b110_114_116_alte_schwierigkeiten.ods",
                         col_names = FALSE)
...
# SC5
items <- c("ics3001x_c", "ics3002s_c", "ics3003x_c", "ics3010x_c", "ics3011x_c",
...
x <- c(1.40366579, -0.86991232, -1.20855800, -0.83436944, -0.90459723,
...
x5 <- data.frame(item=1:length(x), x)
rownames(x5) <- items
...
xsi <- c(-1.78, 1.04, 1.47, -1.82, -0.93, -0.68, -1.32, -0.01, -1.74, -0.64,
...
# SC5
items <- c("scs36310_c", "scs36320_c", "scs36220_c", "scs3623s_c", "scs30510_c",
...
x <- c(0.92237716, 1.46643444, -0.75800539, -0.13650864, -1.07382160,
...
x <- c(0.633, -1.601, -1.774, -1.407, -1.462, -1.634, -0.100, -1.758, -0.628,
         -0.585, -1.073, -1.377, 0.757, -1.306, 0.769, -1.697, -0.984, -2.008,
         -0.501, 0.416, 1.276, -1.721)
x6 <- data.frame(item=1:length(x), x)
rownames(x6) <- items


# store pre-scaled item parameters in list
xsi.fixed$cross[["SC"]] <-
    list(SC4 = list(w5=NULL,
                    w13=NULL),
         SC5 = list(w5=x5),
         SC6 = list(w5=x6))
xsi.fixed$long[["SC"]] <-
...
# xsi <- pcm$all$mod$xsi.fixed.estimated
# xsi <- xsi[!grepl("step", rownames(xsi)), ]
items <- c("efs121010_c", "efs121020_c", "efs121030_c", "efs121040_c",
...
x <- c(-1.18, -0.25, -2.51, -2.02, -0.88, -1.63, -2.26, -2.35, -2.24, -0.21,
...
# xsi <- pcm$all$mod$xsi.fixed.estimated
# xsi <- xsi[!grepl("step", rownames(xsi)), ]
items <- c("bas7mar1_c", "bas7mar2_c", "bas7mar3_c", "bas7mar4_c", "bas7mar5_c",
           ...
           "bas7mac6_c")
x <- c(-1.74, -.21, -1.10, -.87, -.09, -.79, -1.33, -.32, -.74, -.95, -1.36,
