...
#' write.dta(dataframe = xTargetCompetencies_sim,
...
plausible_values <- function(SC,
    ...
    if(is.null(item_labels[[SC]][[domain]][[wave]])) {
        stop(paste0("No competence data available for ", SC, " ", domain, " ",
                    wave, "."))
    }
    if (longitudinal && ((SC == "SC6" | SC == "SC5") &
                         domain %in% c("IC", "SC", "BA", "EF"))){
        longitudinal <- FALSE
        message(paste0("No longitudinal data for: ", SC, " ", domain,
    ...
    if (filetype == "SPSS") {
        if (SC == "SC5" & domain == "BA") {
            data <-
                tryCatch({haven::read_spss(
                    file = paste0(path, files[grep("xEcoCAPI", files)]),
                    user_na = TRUE)},
                    error=function(e){
                        stop(paste0("Path ", path,
    ...
    if (include.nr) {
        sel <- (if (longitudinal) {
            ...
            } else {names(data) %in% item_labels[[SC]][[domain]][[wave]]})
    ...
    if (longitudinal) {
        ...
        data <-
            data[data$ID_t %in% unique(
                unlist(lapply(resp, function(x) {x[["ID_t"]]}))
                ), ]
        data <- data[order(data$ID_t), ]
    } else {
    ...
    PCM <- (if (longitudinal) {
        ...
        else {max(apply(resp[, -1], 2, max, na.rm = TRUE)) > 1})
        ...
        if (rotation) {
            ...
            if (SC == "SC1") {
                ...
                if (wave == "w1") {
                    ...
                    if (domain == "RE") {
                        position[!is.na(position$position) &
                                     (position$position == 126),
                                 "position"] <- 1 # reading first
                        position[!is.na(position$position) &
                                     (position$position == 127),
                                 "position"] <- 2 # reading second
                        position$position <-
                            haven::labelled(position$position,
                                            c("reading first" = 1,
                                              "reading second" = 2))
                    } else if (domain == "MA") {
                        position[!is.na(position$position) &
                                     (position$position == 127),
                                 "position"] <- 1 # maths first
                        position[!is.na(position$position) &
                                     (position$position == 126),
                                 "position"] <- 2 # math second
                        position$position <-
                            haven::labelled(position$position,
                                            c("math first" = 1,
                                              "math second" = 2))
                    ...
                    if (domain == "IC") {
                        position[!is.na(position$position) &
                                 ...
                                 "position"] <- 1 # ict first
                        position[!is.na(position$position) &
                                     (position$position %in% c(330,332,334, 336)),
                                 "position"] <- 2 # ict second
                        position$position <-
                            haven::labelled(position$position,
                                            c("ICT first" = 1,
                                              "ICT second" = 2))
                    ...
                    if (domain == "RE") {
                        ...
                        position[!is.na(position$position) &
                                 ...
                                 "position"] <- 1 # English first
                        position[!is.na(position$position) &
                                     (position$position %in% c(460,462,466,468)),
                                 "position"] <- 2 # English second
                        position$position <-
                            haven::labelled(position$position,
                                            c("English first" = 1,
                                              "English second" = 2))
                ...
                if (wave == "w3") {
                    ...
                    if (domain == "IC") {
                        ...
                        position[!is.na(position$position) &
                                 ...
                                 "position"] <- 1 # no rotation should be found
                    }
                } else if (wave == "w9") {
    ...
    if (longitudinal) {
        ...
        if (rotation) {
            if (PCM) {
                res <- cross_sectional_PCM_rotation(bgdata, imp, frmY = NULL,
                                                    waves, ID_t, resp,
                                                    type, domain, SC, control,
                                                    npv, position)
            } else {
                res <- cross_sectional_rotation(bgdata, imp, frmY = NULL, resp,
    ...
    if (control$WLE) {
        if (longitudinal) {
            ...
            for (j in seq(length(waves))) {
                wmod[[j]] <- TAM::tam.mml.wle2(mod[[j]], WLE = TRUE,
                                               progress = FALSE)
            }
            wmod <- wmod %>%
        ...
        colnames(wle) <-
            c("ID_t",  paste0(rep(c("wle","se"), length(waves)),
                              rep(if (longitudinal) {waves} else {""},
                                  each = 2)))
    }
    datalist <- list()
    d <- 1
    for (i in 1:ifelse(is.null(bgdata) || !any(is.na(bgdata)), 1,
                       control$ML$nmi)) {
        for (j in 1:npv) {
            datalist[[d]] <- pvs[[i]][[j]][, !grepl("pweights|test_postition",
                                                    colnames(pvs[[i]][[j]]))]
        ...
        if (SC == "SC6" && domain == "RE") {
            ...
            for (i in seq(2, length(waves))) {
                longitudinal_IDs[[i-1]] <-
                    dplyr::filter(eap, !is.na(eap[paste0("eap", waves[i-1])]),
                                  !is.na(eap[paste0("eap", waves[i])]))$ID_t
            }
        }
        # re-scaling for longitudinal link
        res <- scale_person_estimates(pv = datalist,
